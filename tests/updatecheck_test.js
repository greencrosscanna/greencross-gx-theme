...
(async () => {
  ...
  let reloads = 0;
  const releases = { ok: true, releases: [{ version: 'v1.771', deployed_at: new Date().toISOString() }] };
  const k = mkKiosk();
  k.win.GXClient = () => ({ jsonp: () => Promise.resolve(releases) });
  k.U.init({ app: 'performance', gxcore: 'https://x/exec', version: () => 'v1.770', autoReload: true });
